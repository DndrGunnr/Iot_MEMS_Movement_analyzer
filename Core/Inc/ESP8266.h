...
typedef struct
{
	...
	uint8_t                       DataBuffer[_WIFI_TX_SIZE];/*PL*/
	uint16_t                      RxIndex;
	bool                          RxIsData;
	//----------------General	Parameter
	WifiMode_t                    Mode;
...
bool Wifi_TcpIp_SendData_Tcp(uint8_t LinkId,uint16_t dataLen,uint8_t *data);
bool Wifi_TcpIp_SendData_Udp(uint8_t LinkId,uint16_t dataLen,uint8_t *data);
